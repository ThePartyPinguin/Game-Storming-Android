...
namespace GameFrame.Networking.NetworkConnector
{
    public class NetworkConnector<TEnum> where TEnum : Enum
    {
        ...
        private readonly Boo.Lang.List<TEnum> _allowedEvents;
        ...
        public NetworkConnector(IPAddress ipAddress, int tcpReceivePort, int udpRemoteSendPort, int udpReceivePort)
        {
            ...
            _allowedEvents = new Boo.Lang.List<TEnum>();
        }
        ...
        {
            if (!_allowedEvents.Contains(allowedEvent))
                _allowedEvents.Add(allowedEvent);
        }

        public void AddMultipleAllowedEvents(IEnumerable<TEnum> allowedEvents)
        {
            foreach (var tEnum in allowedEvents)
            {
                if (!_allowedEvents.Contains(tEnum))
                    _allowedEvents.Add(tEnum);
            }
        }
        public void RemoveAllowedEvent(TEnum allowedEvent)
        {
            if (_allowedEvents.Contains(allowedEvent))
                _allowedEvents.Remove(allowedEvent);
        }

        public void RemoveMultipleAllowedEvents(IEnumerable<TEnum> allowedEvents)
        {
            foreach (var tEnum in allowedEvents)
            {
                if (_allowedEvents.Contains(tEnum))
                    _allowedEvents.Remove(tEnum);
            }
        }

        public bool IsAllowToReceiveEvent(TEnum receivedEvent)
        {
            return _allowedEvents.Contains(receivedEvent);
        }
        /// <summary>
